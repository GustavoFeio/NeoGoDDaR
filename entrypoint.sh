...
usage() {
    ...
    echo "  docker run -it neogoddar [COMMAND] [OPTIONS]"
    echo ""
    ...
    echo "  docker run -it neogoddar benchmark-bugs"
    echo ""
    echo "  # Same, but print results live to the terminal"
    echo "  docker run -it neogoddar benchmark-bugs --terminal"
    echo ""
    echo "  # Run validation suite with verbose intermediate representation"
    echo "  docker run -it neogoddar benchmark-examples --verbose"
    echo ""
    ...
    echo "  docker run -it neogoddar analyse-ccs 'a!.b?.0 || b!.a?.0'"
    echo ""
    echo "  # Clean all generated *.output files from a previous run"
    echo "  docker run -it neogoddar benchmark-all --clean"
    echo ""
