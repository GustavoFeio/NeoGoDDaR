...
let deadlock_solver_2_dfs (go_fixer_fmt: formatter option) (lambda: LambdaTagged.t) (deadlocked_top_environment: EtaTagged.eta list): (LambdaTagged.t) =
  ...
  let rec do_deadlock_solver_2  (go_fixer_fmt: formatter option) (lambda: LambdaTagged.t): (LambdaTagged.t) =
    ...
    let is_co_input c1 ((eta, found_co_output): (EtaTagged.eta * EtaTagged.eta option)) =
      ...
      | (EEta(AClose(_),_), _) -> failwith "do_deadlock_solver_2: received AClose"
      | (EEta(ACall(_),_), _) -> failwith "do_deadlock_solver_2: received ACall"
    in
    match lambda with
...
let rec detect_and_resolve fmt (go_fixer_fmt: formatter option) eval (procs: func_def list) exp (deps: dependency list): errors * LambdaTagged.t =
  ...
  let errors = (eval fmt procs exp) in
  Format.fprintf fmt "\n";
  ...
  let best_eta = (
    if !Cmd.all_etas then (
      let best_etas = Heuristic_NOP.best_etas exp errors.states in
      Heuristic_NOP.print_eta_score debug_fmt best_etas;
      best_etas
      |> List.map (fun (eta, _) -> eta)
    ) else (
      (* let best_etas = Heuristic_by_layer.best_etas exp errors.states in
      Heuristic_by_layer.print_eta_score debug_fmt best_etas;
      Heuristic_by_layer.best_eta exp best_etas *)
      let best_etas = Heuristic_New.best_etas exp errors.states in
...
let rec detect_and_resolve_loop (go_fixer_fmt: formatter option) eval (procs: func_def list) (errors, resolved) deps (last_resolved: LambdaTagged.t list) =
    match last_resolved with
    | [] when errors.states = [] -> 
      (true, errors, resolved)
    (* When resolved program remains the same then exit loop*)
    | _ when List.mem resolved last_resolved ->
      (errors.states = [], errors, resolved)
    (* When no deadlocks are found then exit loop*)
    | _ -> 
      let res = detect_and_resolve null_fmt go_fixer_fmt eval procs resolved deps in
      detect_and_resolve_loop go_fixer_fmt eval procs res deps (resolved::last_resolved)
