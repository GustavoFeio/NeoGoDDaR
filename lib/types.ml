...
type chan = string * chan_attr
type func = string * chan list
...
let rec print_params fmt (params: chan list): unit =
    match params with
    | [] -> ()
    | [(c, _)] -> fprintf fmt "%s" c
    | (c, _)::tl -> fprintf fmt "%s, " c; print_params fmt tl

let print_action fmt a =
    match a with
    | AIn(label, _) -> fprintf fmt "AIn(%s)" label
    | AOut(label, _) -> fprintf fmt "AOut(%s)" label
    | AClose(label, _) -> fprintf fmt "AClose(%s)" label
    | ACall(fun_name, args) -> fprintf fmt "ACall(%s," fun_name; print_params fmt args; fprintf fmt ")"

let print_action_simple fmt a =
    match a with
    | AIn(label, _) -> fprintf fmt "%s?" label
    | AOut(label, _) -> fprintf fmt "%s!" label
    | AClose(label, _) -> fprintf fmt "#%s" label
    | ACall(fun_name, args) -> fprintf fmt "%s[" fun_name; print_params fmt args; fprintf fmt "]"
...
module Eta =
  ...
  let compl_action action = 
      ...
      | AIn(k) -> AOut(k)
      | AOut(k) -> AIn(k)
      (* FIXME: This was only done to compile the program during early development. It may not be correct in the long term *)
      | AClose(k) -> AClose(k)
      | ACall(k) -> ACall(k)
  ...
  let is_compl_action action1 action2: bool =
      match action1, action2 with
      | AIn(c1,_), AOut(c2,_)
      | AOut(c1,_), AIn(c2,_) when c1 = c2 -> true
      | _ -> false
  end
...
type func_def = string * chan list * LambdaTagged.t

let print_proc fmt ((fun_def, params, exp): func_def): unit =
    fprintf fmt "%s[" fun_def; print_params fmt params; fprintf fmt "] ::= %a;\n" LambdaTagged.print exp

(* ---------- Functions ----------  *)

let rec lambdaToLambdaTagged (procs, exp: (string * chan list * Lambda.t) list * Lambda.t): func_def list * LambdaTagged.t = 
    let i = ref 0 in
    ...
    in ((List.map (fun (fun_name, params, stmts) -> fun_name, params, do_lambdaToLambdaTagged stmts) procs), do_lambdaToLambdaTagged exp)
...
let actionToString action: string = 
  ...
  | AIn(label, _) -> label ^ "?"
  | AOut(label, _) -> label ^ "!"
  | AClose(label, _) -> "#" ^ label
  | ACall(fun_name, _) -> "%" ^ fun_name
...
type error =
    { channel : chan;
      ctx : ctx;
      (* TODO: There are a few options for storing where the error occurred:
             The EEtaTagged type already stores a `tag` which holds a location for the Go code (depends on the MiGo code gen).
             We could make it a channel attribute (debatable since it was the action that caused the error, not the channel).
             We could store it as an extra field in this `error` record.
         The first two would make this record useless unless more fields are added. *)
    }

type errors =
    { states: state list;
      double_close: error list;
      closed_write: error list;
    }

let no_errors: errors =
    { states = [];
      double_close = [];
      closed_write = [];
    }
